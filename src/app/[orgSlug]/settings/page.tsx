...
export default function OrgSettingsPage() {
    ...
    const performEmailUpdate = async () => {
        if (!user) return;
        try {
            // verifyBeforeUpdateEmail is the modern, secure way to change emails.
            // It sends a verification link to the NEW email address.
            // The email won't actually change in Firebase until the user clicks that link.
            await verifyBeforeUpdateEmail(user, email);
            setMessage({ 
                type: "success", 
                text: "A verification link has been sent to your new email address. Please click it to complete the change." 
            });
        } catch (error: any) {
            ...
            if (error.code === "auth/operation-not-allowed") {
                setMessage({ 
                    type: "error", 
                    text: "Email updates are currently restricted. Please ensure 'Email Link' is enabled in Firebase Console > Authentication > Settings." 
                });
            } else {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            <div>
                ...
                <p className="text-muted-foreground">Manage your credentials and security settings.</p>
            </div>
                ...
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-slate-600 dark:text-slate-400 pt-2">
                            Are you sure you want to change your account email to:
                    ...
                    <DialogFooter className="gap-2 sm:gap-0">
                        ...
                        <Button onClick={handleConfirmEmail} className="bg-amber-600 hover:bg-amber-700 text-white border-none shadow-lg shadow-amber-600/20">
                            Yes, I'm Sure
                        </Button>
                    </DialogFooter>
                </DialogContent>
